...
function convertUrlToDeepLink(
  ...
  if (isDeepLinkType(routeName, params, DeepLink.MagicLink)) {
    return `aispec://auth/signIn/${encodeURIComponent(url)}/${params?.tenantId}/${params?.email}`;
  } else if (isDeepLinkType(routeName, params, DeepLink.ForgotPassword)) {
    return `aispec://auth/createNewPassword/${params?.oobCode}/${params?.tenantId}/${params?.email}`;
  }
  return undefined;
}
